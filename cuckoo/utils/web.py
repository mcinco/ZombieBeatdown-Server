...
def whitelistindex():
    ...
    rows = db.list_tasks()
    for row in rows:
        filename = re.sub(r'.*/', '', row.target)
        task = {
            "id" : row.id,
            "status" : row.status,
            "filename" : filename,
        }
        whitelists.append(task)
    template = env.get_template("whitelistindex.html")
    return template.render({"rows" : whitelists})

@route("/whitelist", method="POST")
def whitelist():
    context = {}
    taskid = []
    numberofanalysis = request.forms.get("buttonvalue", "")
    for i in range(0, int(numberofanalysis)):
        taskid.append(request.forms.get("dropdownbox"+str(i), ""))
    #print "./common.sh " + " ".join(taskid)
    os.system("bash common.sh " + " ".join(taskid))
