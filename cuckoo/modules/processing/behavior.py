...
class Summary:
    ...
    def list_difference(self, whitelist, filelist):
        c = list(filelist)
    ...
    def run(self):
        ...
        """
        with open(self._log_path + "/fileslist", "w") as f:
            f.write("\n".join(self.files))
        with open(self._log_path + "/registrylist","w") as f:
            f.write("\n".join(self.keys))
        whitelistpath = os.path.join(CUCKOO_ROOT, "whitelist")
        filteredfiles = []
        filteredkeys = []
        filteredfiles = self.list_difference(whitelistpath + "/commonfile.txt", self.files)
        filteredkeys = self.list_difference(whitelistpath + "/commonreg.txt", self.keys)



        return {"files": filteredfiles, "keys": filteredkeys, "mutexes": self.mutexes}
